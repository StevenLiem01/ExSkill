      ...
      <main className="min-h-screen bg-slate-900 text-slate-50 p-6 md:p-12 relative overflow-hidden pb-20">
        ...
        <div className="max-w-4xl mx-auto space-y-8 relative z-10">
          ...
          <div className="flex border-b border-white/10 w-full mb-6 relative z-50">
            <Link 
              href="?tab=incoming" 
              className={`flex-1 text-center py-4 text-sm font-bold uppercase tracking-wider transition-colors border-b-2 ${activeTab === 'incoming' ? 'text-[#00DF9A] border-[#00DF9A]' : 'text-slate-500 border-transparent hover:text-slate-300'}`}
            >
              📩 Proposal Masuk ({incoming.length})
            </Link>
            <Link 
              href="?tab=outgoing" 
              className={`flex-1 text-center py-4 text-sm font-bold uppercase tracking-wider transition-colors border-b-2 ${activeTab === 'outgoing' ? 'text-[#00DF9A] border-[#00DF9A]' : 'text-slate-500 border-transparent hover:text-slate-300'}`}
            >
              📤 Proposal Terkirim ({outgoing.length})
            </Link>
          </div>
            ...
            {activeTab === 'incoming' && (
              <div className="space-y-6">
                {incoming.length === 0 ? (
                  ...
                  <div key={p.id} className="bg-slate-800/50 backdrop-blur-md border border-white/10 rounded-2xl p-6 md:p-8 shadow-sm hover:border-[#00DF9A]/50 transition-all duration-300 ease-in-out flex flex-col gap-6 relative">
                    ...
                    <div className="bg-slate-900/50 p-5 rounded-xl border border-white/5 text-sm space-y-4 shadow-inner grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <span className="font-semibold text-[#00DF9A] bg-[#00DF9A]/10 px-4 py-2 rounded-lg border border-[#00DF9A]/30 w-fit shadow-sm">{p.offered_skill.name}</span>
                      </div>
                    </div>
            ...
            {activeTab === 'outgoing' && (
              <div className="space-y-6">
                {outgoing.length === 0 ? (
                  ...
                  <div key={p.id} className="bg-slate-800/50 backdrop-blur-md border border-white/10 rounded-2xl p-6 md:p-8 shadow-sm hover:border-[#00DF9A]/50 transition-all duration-300 ease-in-out flex flex-col gap-6 relative opacity-90 hover:opacity-100">
                    <div className="absolute top-6 right-6">
                      <StatusBadge status={p.status} />
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <Link href={`/users/${p.receiver.id}`} className="font-bold text-xl text-white hover:text-[#00DF9A] transition-colors">{p.receiver.name}</Link>
                        <p className="text-xs text-slate-400 font-medium flex items-center gap-1 mt-0.5">
                    ...
                    <div className="bg-slate-900/50 p-5 rounded-xl border border-white/5 text-sm space-y-4 shadow-inner grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="flex flex-col gap-2">
                        ...
                        <span className="font-semibold text-[#00DF9A] bg-[#00DF9A]/10 px-4 py-2 rounded-lg border border-[#00DF9A]/30 w-fit shadow-sm">{p.requested_skill.name}</span>
                      </div>
                      <div className="flex flex-col gap-2">
                    ...
                    <p className="text-xs text-slate-400 font-medium mt-2 pt-4 border-t border-white/5">
                      📅 Dikirim pada: {new Date(p.created_at).toLocaleDateString('id-ID', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                    </p>
                  </div>
                ))}
              </div>
